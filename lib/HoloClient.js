...
HoloClient.prototype.buildAPIWithConfig = function(endpoints) {
  ...
  endpoints.forEach(function (endpoint) {
    ...
    level[name] = function() {
      ...
      var requiredParams =  params.filter((param) => param.required );
      var optionalParams =  params.filter((param) => !param.required );
      
      // put values from method arguments on the item.
      requiredParams.forEach((rParam, idx) => {
        if (_.isUndefined(rParam.value)) {
          rParam.value = arguments[idx];
        }
      });

      var optionValues = arguments[arguments.length - 1];
      optionValues = _.pick(optionValues, optionalParams.map((oParam) => oParam.name));

      optionalParams.forEach((oParam) => {
...
HoloClient.prototype.get = function(fqdn, debug) {
  return new Promise(function(resolve, reject) {
    ...
    if (this.config.showDebug || debug) console.log(`GET: ${fqdn}`);
...
HoloClient.prototype.post = function(fqdn, body = {}, debug) {
  return new Promise(function(resolve, reject) {
    ...
    if (this.config.showDebug || debug) console.log(`POST: ${fqdn} with ${JSON.stringify(body)}`);
...
HoloClient.prototype.put = function(fqdn, body = {}, debug) {
  return new Promise(function(resolve, reject) {
    ...
    if (this.config.showDebug || debug) console.log(`PUT: ${fqdn} with ${JSON.stringify(body)}`);
...
HoloClient.prototype.delete = function(fqdn, body = {}, debug) {
  return new Promise(function(resolve, reject) {
    ...
    if (this.config.showDebug || debug) console.log(`DELETE: ${fqdn} with ${JSON.stringify(body)}`);
